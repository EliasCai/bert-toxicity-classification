...
python run_classifier.py \
  --data_dir=/data1/1904_toxicity-classification/input/ --vocab_file=/data1/1904_awesone-bert/uncased_L-12_H-768_A-12/vocab.txt \
  --bert_config_file=/data1/1904_awesone-bert/uncased_L-12_H-768_A-12/bert_config.json \
  --init_checkpoint=/data1/1904_awesone-bert/uncased_L-12_H-768_A-12/bert_model.ckpt \
  --task_name=toxic \
  --do_train=True \
  --do_eval=True \
  --do_predict=True \
    ...
    def _read_csv(cls, input_file, quotechar='"'):
        ...
        with open(input_file, "r") as f:
            ...
            for idx, line in enumerate(reader):
                # if os.path.basename(input_file) == 'test.csv' and idx == 0:
                    # print(line)
                    # continue # Ignore the first line
                # if idx < 10:
                    # print(line)
                lines.append(line)
            # print('len of', input_file, len(lines))
            return lines
...
class ToxicProcessor(DataProcessor):
    ...
    # sudo cat train.csv | tail  -n +2 | head -n 10 | shuf
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, "dev_1000.csv")), "dev"
        )
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            # Only the test set has a header
            if set_type in ["train","test"] and i == 0:
                continue
            guid = "%s-%s" % (set_type, i)
            if set_type == "test":
                text = line[1]
                ...
                try:
                    ...
                    print(len(line))
                    print(line)
                l = int(float(line[1]) > 0.5)
                label = tokenization.convert_to_unicode(str(l))
            labels.append(label)
            labels_test.append(label)
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label)
            )
        # print('len of example', len(examples))
        return examples, labels, labels_test
...
def main(_):
    ...
    if FLAGS.do_predict:
        predict_examples, truth_labels, placeholder_labels = processor.get_test_examples(
            FLAGS.data_dir
        )
        ...
        # example to features and then serialise to string and put into file
        label_map_test = file_based_convert_examples_to_features(
            predict_examples,
            label_list_test,
